...
class BulkEmployeeServiceContractForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $param_shift_scheduler = $request->query->get('shift_scheduler');

    $shift_scheduler = $this->entityTypeManager->getStorage('sub_shift_scheduler')->load($param_shift_scheduler);

    $required_employee = $shift_scheduler->get('field_required_number_employees')->getString();
    $employee_count_options = array_combine(range(1, $required_employee), range(1, $required_employee));
    
    $form['select_required_employee'] = [
      '#type' => 'select',
      '#title' => $this->t('Orders Assignment'),
      '#options' => $employee_count_options,
      '#empty_option' => $this->t('- Select a number -'),
      '#required' => TRUE,
      '#ajax' => [
        'callback' => '::updateAssignments',
        'wrapper' => 'assignments-wrapper',
      ],
    ];
  
    $form['assignments_container'] = [
      '#type' => 'container',
      '#attributes' => ['id' => 'assignments-wrapper'],
    ];

    $selected_employee_count = $form_state->getValue('select_required_employee');
    if (!empty($selected_employee_count)) {
      ...
      $assignments = $this->entityTypeManager->getStorage('sub_assignment')->loadByProperties([
        ...
        'f_assignment_order' => $selected_employee_count,
      ]);
      //dump($assignments);
      ...
      $form['assignments_container']['address'] = [
      ...
      $form['assignments_container']['time'] = [
          '#markup' => "<div class='header-time'>$time</div>",
      ];
      $form['assignments_container']['dates'] = [
          '#markup' => "<div class='header-dates'>$dates</div>",
      ];
  
      $form['assignments_container']['employee_select'] = [
        '#type' => 'select',
      ...
      if (!empty(($employee_options))) {
        $form['assignments_container']['related_plan'] = [
          '#type' => 'markup',
      ...
      $form['assignments_container']['employee_schedule'] = [
        '#markup' => $output,
      ];
  
      $form['assignments_container']['markup_hidden'] = [
        '#type' => 'hidden',
        '#value' => $output,
      ];
  
      $form['assignments_container']['actions'] = [
        '#type' => 'container',
        '#attributes' => ['class' => 'form-actions'],
      ];
  
      $form['assignments_container']['actions']['submit'] = [
        '#type' => 'submit',
        '#value' => t('Bulk Assign'),
      ];
    } 
    else {
      $form['assignments_container']['assignments_container']['assignments'] = [
        '#markup' => $this->t('Please select the order assignment'),
      ];
    }
   ...
   * Ajax callback 
   */
  public function updateAssignments(array &$form, FormStateInterface $form_state) {
    return $form['assignments_container'];
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $order_assignment = $form_state->getValue('select_required_employee');
    $shift_scheduler_id = $request->query->get('shift_scheduler');
    ...
    $form_state->setRedirectUrl(\Drupal\Core\Url::fromUri('internal:/service-contracts'));
